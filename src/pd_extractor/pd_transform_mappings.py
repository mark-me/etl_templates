...
class TransformMappings(ObjectTransformer):
    ...
    def __mapping_compositions(
        ...
        """Cleans the composition of source entities data
            ...
            In this function we start out with a list of compositions because each mapping contains an example
            composition generated by a MDDE extension. We remove these examples and assume we only have to deal with
            one composition per mapping. That is why the function switches from a list of compositions to handling only
            a composition.
    ...
    def compositions_remove_mdde_examples(self, lst_compositions: list) -> dict:
        """Removing MDDE example composition, assuming one composition remains

        Args:
            lst_compositions (list): Compositions, including MDDE example composition
